...
df = pd.read_csv('/Users/svenjaschmiedl/Documents/Studium/Leipzig_Master/2. Semester/BD_Pr/Daten/GerryWeber.csv')
...
df["name"] = df["name"].apply(lambda x: x.replace('1/2', 'einhalb'))
df["name"] = df["name"].apply(lambda x: x.replace('3/4', 'dreiviertel'))
df["name"] = df["name"].apply(lambda x: x.replace('7/8', 'siebenachtel'))
#df["name"] = df["name"].apply(lambda x: x.replace('4', 'for')) #sinnvoll?
#df["name"] = df["name"].apply(lambda x: re.sub('[0-9]+','', x))
df["name"] = df["name"].apply(lambda x: x.replace('  ', ' '))

df = df.assign(Marke="gerry weber")
