...
def blocks_statistics(zal_dict, th_gw_dict):
    """
    ...
    numA_blocks = len(zal_dict)
    numB_blocks = len(th_gw_dict)

    block_sizeA_list = []
    for rec_id_list in zal_dict.values():  # Loop over all blocks
        block_sizeA_list.append(len(rec_id_list))

    block_sizeB_list = []
    for rec_id_list in th_gw_dict.values():  # Loop over all blocks
        block_sizeB_list.append(len(rec_id_list))

    logging.info('Dataset Zalando number of blocks generated: %d' % numA_blocks)
    logging.info('    Minimum block size: %d' % (min(block_sizeA_list)))
    logging.info('    Average block size: %.2f' % \
                 (float(sum(block_sizeA_list)) / len(block_sizeA_list)))
    logging.info('    Median block size: %d' % (median(block_sizeA_list)))
    logging.info('    Maximum block size: %d' % (max(block_sizeA_list)))
    logging.info('')

    logging.info('Dataset TommyHilfiger/GerryWeber number of blocks generated: %d' % numB_blocks)
    logging.info('    Minimum block size: %d' % (min(block_sizeB_list)))
    logging.info('    Average block size: %.2f' % \
                 (float(sum(block_sizeB_list)) / len(block_sizeB_list)))
    logging.info('    Median block size: %d' % (median(block_sizeB_list)))
    logging.info('    Maximum block size: %d' % (max(block_sizeB_list)))
    logging.info('')
...
class MatchingUtilities:
    ...
    def create_potential_matches(self):
        ...
        logging.info(f'Number of potential classified_matches:             {cnt}')
